...
__assets_base_path = 'https://assets.msn.com/service/Finance/{}?apikey=0QfOX3Vn51YCzitbLaRkTTBadtWpgTN8NZLW0C1SEM&ocid=finance-finance-Peregrine&market=zh-cn&ids={}&wrapodata=false&{}'
...
def __get(action, ids, param=''):
    rsp = __session.get(__assets_base_path.format(action, ','.join(ids), param), headers=headers, timeout=timeout)
...
def lists(symbols):
    return __get('Quotes', symbols, 'fdhead=' + ','.join(__fhead))


def history(symbol):
    return __get('Charts', [symbol], 'type=1Y&disableSymbol=false')
